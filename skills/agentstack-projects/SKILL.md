...
Enables creating and managing AgentStack projects and API keys from Claude Code via MCP tools under `projects.*`.
...
| `apikeys.list` | List API keys (values masked). | Key names and ids. |
| `apikeys.create` | Create API key; value shown once only. | New key — save immediately. |
...
1. **First-time / try it:** Call `projects.create_project_anonymous` with `params: { "name": "My Project" }`. Use `project_api_key` or `user_api_key` as `X-API-Key` in Claude Code MCP config so the agent can call other tools for this project.
...
4. **API keys:** Use `apikeys.create` to create a key (display once); use `apikeys.list` to list (values masked); use `apikeys.delete` to delete.
...
- "Create a new API key for project 1025 named Production" → `apikeys.create` with `project_id: 1025`, `name: "Production"`.
...
- **MCP_CAPABILITY_MATRIX** — generated from `GET /mcp/actions`; use it for exact action names, counts, required caps, and parameters.
