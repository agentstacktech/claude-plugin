...
|-----------------------------------------------------|--------------------------------------------------------------------|
| "try without signup / no API key yet" (key-based clients) | `projects.create_project_anonymous` — top-level `user_api_key` or `session_token` + neutral `bootstrap` metadata; configure MCP client once (see `docs/plugins/CONTEXT_FOR_AI_MCP.md` § Anonymous bootstrap). **Claude Code:** use `GET /mcp/actions` + `agentstack.execute` instead. |
...
| "create an API key for my CI job / Claude Code agent" | `apikeys.create` **or** `user.apikeys.create` with `actor_kind=agent`, `preset=agent_runner`, and `context.project_id` |
...
Claude Code `GET /mcp/actions` + `agentstack.execute` issues a **user-scoped** Bearer (ecosystem `project_id=1`). For any session-scoped work on a tenant project, pass **`context.project_id`** on `agentstack.execute`:
...
- **DO NOT** copy-paste a key from the dashboard — use **OAuth Device Code** (`GET /mcp/actions` + `agentstack.execute` or `GET /mcp/actions` + `agentstack.execute`), which writes a scoped Bearer into `Claude MCP (`claude mcp list`)`.
...
- Live action catalog (filter `projects.*`, `apikeys.*`): `GET https://agentstack.tech/mcp/actions` or run `GET /mcp/actions` + `agentstack.execute`.
...
- OAuth Device Code scopes (as issued by `GET /mcp/actions` + `agentstack.execute`): `mcp:execute mcp:read projects:write projects:admin 8dna:write logic:write rag:write bots:admin buffs:read buffs:write apikeys:write`. A write-capable plugin token also expands `project_admin` / `mcp_read` at MCP L1 without re-auth after Core deploy.
